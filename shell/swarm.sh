    ...
    then 
        ...
        then
            ...
            then
                ...
                if [[ $scp_rc -eq 0 ]]
                then
                    cargo=$(sshpass -p "$mypass" ssh -tt -l $user -p $sshport -q -o ConnectTimeout=5 -o StrictHostKeyChecking=no $extra_ssh_opts $job_server "openssl enc -base64 -aes-256-cbc -d -in /tmp/$shadow_filename -k $random_key | sudo -p \"\" -S su - root -c \"$command\"; rc=\$?; test -f /tmp/$shadow_filename && rm /tmp/$shadow_filename; exit \$rc" 2>/dev/null)
                fi
            else
